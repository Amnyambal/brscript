(function () {
    ...
    try {
        (function() {
            ...
            const DATA_TECH_COMPLAINT = [
                ...
                { text: 'CHEREPOVETS (89)', link: 'https://forum.blackrussia.online/forums/Жалобы-на-игроков.3967/', color: '#0000CD' },
            ];
            ...
            // --- DRAG & DROP ---
            let pos = JSON.parse(localStorage.getItem(STORAGE_PREFIX + 'pos')) || {x: window.innerWidth - 60, y: window.innerHeight * 0.6};
            ...
            updatePos(pos.x, pos.y);
            renderMenu();
            if(localStorage.getItem(STORAGE_PREFIX + 'state') === 'true') toggleMenu(true);
    ...
    // ========================================================================
    // МОДУЛЬ 2: БЫСТРЫЕ ОТВЕТЫ (RELIABLE BUTTONS)
    // ========================================================================
    try {
        (function () {
            ...
            const init = () => {
                ...
                const addButton = (name, id, style) => {
                    ...
                    btn.onclick = () => {
                        // Logic map
                        if (id === 'unaccept') editThreadData(UNACCEPT_PREFIX, false);
                        if (id === 'pin') editThreadData(PIN_PREFIX, true);
                        if (id === 'teamProject') editThreadData(COMMAND_PREFIX, true);
                        if (id === 'watched') editThreadData(WATCHED_PREFIX, false);
                        if (id === 'decided') editThreadData(DECIDED_PREFIX, false);
                        if (id === 'closed') editThreadData(CLOSE_PREFIX, false);
                        if (id === 'odobreno') editThreadData(ODOBRENO_PREFIX, false);
                        if (id === 'techspec') editThreadData(TECHADM_PREFIX, true);
                    };

                    const target = document.querySelector('.button--icon--reply');
                    if (target) target.before(btn);
                };

                // Original Styles
                addButton('На рассмотрение', 'pin', 'border-radius: 20px; margin-right: 11px; border: 2px solid; border-color: rgb(255, 165, 0);');
                addButton('КП', 'teamProject', 'border-radius: 20px; margin-right: 10px; border: 2px solid; border-color: rgb(255, 255, 0);');
                    ...
                    btn.onclick = () => {
                        ...
                        setTimeout(() => {
                            buttons.forEach((btn, i) => {
                                ...
                                if(b) b.onclick = () => pasteContent(i, i > 6); // Logic from old script
                            });
                        }, 200);
                    };
                    const target = document.querySelector('.button--icon--reply');
            ...
            // Persistent Loader (Checks every second until buttons exist)
            ...
            function getThreadData() {
                ...
                return {
                    user: {
                        id: author ? author.getAttribute('data-user-id') : 0,
                        name: author ? author.innerText : 'User',
                        mention: author ? `[USER=${author.getAttribute('data-user-id')}]${author.innerText}[/USER]` : ''
                    },
            ...
            function editThreadData(prefix, pin = false) {
                ...
                formData.append('title', title ? title.innerText : '');
